...
#let proof(body, pre: "Proof", count: c_thm, indent: none) = {
  context v(dingli-inpackage-vertical-padding.get().at(0), weak: true)
  block[
  #if indent != none {
  parvirtual
...
// derived styles
#let lemma = theorem.with(kind: "lemma", pre: "Lemma", count: c_lem)
#let corollary = theorem.with(kind: "corollary", pre: "Corollary", count: c_cor)
#let definition = theorem.with(kind: "definition", pre: "Definition", count: c_def)
#let example = theorem.with(kind: "example", pre: "Example", count: c_xmp)


#let dingli = theorem.with(pre: "定理", count: c_thm, indent: true)
