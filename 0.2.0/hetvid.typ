...
#let hetvid(
    ...
    // For multi-author (array), extract name strings; otherwise use as-is.
    let doc-author = if type(author) == array {
      author.map(a => {
        if type(a.name) == str { a.name } else { repr(a.name) }
      })
    } else {
      author
    }
    set document(title: title, author: doc-author)
